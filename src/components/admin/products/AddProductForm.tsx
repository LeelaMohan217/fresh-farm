...
type CategoryGroup = { parentId: number; parentName: string; subs: { id: number; name: string }[] };
...
export default function AddProductForm({
  ...
  const allSubs = categoryGroups.flatMap((g) => g.subs);
  ...
  const [form, setForm] = useState({
    ...
    price: "",
    unit: "kg",
    categoryId: allSubs[0]?.id?.toString() ?? "",
    stock: "",
  ...
  const handleFilesChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Build entries with local previews
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!form.categoryId) errs.categoryId = "Please select a sub-category.";
  ...
  if (success) {
    return (
      <div className="max-w-2xl mx-auto flex flex-col items-center justify-center py-24 text-center space-y-3">
        ...
        <p className="text-lg font-bold text-slate-900">Product added successfully!</p>
    ...
    <div className="h-full flex flex-col max-w-4xl mx-auto">
      ...
      <div className="shrink-0 flex items-center gap-3 pb-5">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">List a new item in your catalogue</p>
        </div>
      </div>

      {/* ── Scrollable form ── */}
      <div className="flex-1 overflow-y-auto scrollbar-none min-h-0 -mr-6 pr-6">
      <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              {categoryGroups.length > 0 ? (
                <>
                  <CustomSelect
                    ...
                    groups={categoryGroups.map((g) => ({
                      label: g.parentName,
                      options: g.subs.map((s) => ({ value: String(s.id), label: s.name })),
                    }))}
                  />
                ...
                <div className="h-10 px-3 flex items-center text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-lg">
                  No sub-categories —{" "}
                  <a href="/admin/products/categories" className="underline ml-1">add categories first</a>
                </div>
              )}
            </div>
            <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-3 pb-6">
          <button
            type="submit"
            disabled={loading || anyUploading || allSubs.length === 0}
