...
import {
  Search, PackageSearch, Download,
  ChevronLeft, ChevronRight, Trash2, Loader2, RotateCcw,
} from "lucide-react";
...
export type BulkOrderRow = {
  ...
  bookingDate: string;
  deliveryDate: string;
  status: "Confirmed" | "Pending" | "Processing" | "Cancelled";
  price: number;
};

const PAGE_SIZE = 25;
const STATUSES = ["All", "Pending", "Confirmed", "Processing", "Cancelled"] as const;
type FilterStatus = (typeof STATUSES)[number];

const STATUS_STYLE: Record<BulkOrderRow["status"], string> = {
...
export default function BulkOrdersTable({ orders: initial, currentAdminRole }: { orders: BulkOrderRow[]; currentAdminRole: string }) {
  ...
  const [confirmTarget, setConfirmTarget] = useState<BulkOrderRow | null>(null);
  ...
  return (
    <div className="flex flex-col h-full max-w-7xl mx-auto gap-5">
      <ConfirmDialog
        ...
        description={`Delete bulk order "${confirmTarget?.id}"? This will permanently remove the order and all its items.`}
        confirmLabel="Delete"
        loading={!!deletingId}
      ...
      <div className="shrink-0 space-y-4">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-slate-900">Bulk Orders</h1>
            <p className="text-sm text-slate-400 mt-0.5">{orders.length} total bulk orders</p>
          </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-none rounded-xl border border-slate-100 bg-white">
        {paginated.length > 0 ? (
          <table className="w-full text-sm border-separate border-spacing-0">
            <thead className="sticky top-0 z-10">
              <tr className="border-b border-slate-100 bg-slate-50/95 backdrop-blur-sm">
                ...
                <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 tracking-wide">Event</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 tracking-wide">Delivery</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {paginated.map((order) => (
                ...
                >
                  ...
                  <td className="px-5 py-3.5 text-slate-600 max-w-45 truncate">{order.items}</td>
                  <td className="px-5 py-3.5 text-slate-700 font-medium">{order.user}</td>
                  <td className="px-5 py-3.5">
                    <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${eventStyle(order.eventType)}`}>
                      {order.eventType}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-slate-500 text-xs">{order.deliveryDate}</td>
                  <td className="px-5 py-3.5 font-semibold text-slate-800">₹{order.price.toLocaleString("en-IN")}</td>
                  <td className="px-5 py-3.5">
                    <span className={`text-[11px] font-medium px-2.5 py-0.5 rounded-full ${STATUS_STYLE[order.status]}`}>
                      {order.status}
                    </span>
                  </td>
          ...
          <div className="h-full flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm font-medium text-slate-700">No bulk orders found</p>
      ...
      {filtered.length > 0 && (
        <div className="shrink-0 flex items-center justify-between text-xs text-slate-400">
          <p>Showing {from}–{to} of {filtered.length} bulk orders</p>
